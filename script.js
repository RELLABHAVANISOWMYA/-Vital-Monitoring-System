document.addEventListener('DOMContentLoaded', () => {
    ...
    function validateSlide3() {
        ...
        if (temperature && temperatureUnit) {
            ...
            if (!temperature) document.getElementById('temperatureError').style.display = 'block';
            if (!temperatureUnit) document.getElementById('temperatureUnitError').style.display = 'block';
        }
    }
    ...
    function checkHealth() {
        ...
        if (heartRate < heartRateNormalRange[0] || heartRate > heartRateNormalRange[1]) {
            ...
            recommendedTests.push("Electrocardiogram (ECG): To check for heart rhythm issues.", "Holter Monitor: To track heart activity over time.");
        }
        ...
        if (isNaN(systolic) || isNaN(diastolic)) {
            ...
            recommendedTests.push("Ambulatory Blood Pressure Monitoring (ABPM): For 24-hour blood pressure tracking.", "Electrocardiogram (ECG): To check for heart issues.");
        } else if (systolic < bloodPressureNormal.systolic[0] || diastolic < bloodPressureNormal.diastolic[0]) {
            alertMessage += "Blood Pressure is low. ";
            recommendedTests.push("Blood Tests: To identify possible causes.", "Tilt Table Test: To diagnose fainting and dizziness issues.");
        }

        if (temperature < tempRange[0] || temperature > tempRange[1]) {
            if (temperature > tempRange[1]) {
                alertMessage += "Body Temperature is high. ";
                recommendedTests.push("Blood Tests: To check for infections or inflammation.", "Chest X-ray: To examine potential lung issues.");
            } else {
                alertMessage += "Body Temperature is low. ";
                recommendedTests.push("Blood Tests: To check for underlying health issues.", "Electrolyte Tests: To evaluate electrolyte imbalance.");
            }
        }
